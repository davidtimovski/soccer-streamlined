...
class App {
    ...
    for (let match of matches) {
      ...
      matchButton.addEventListener(
        ...
        () => {
          ...
          let kickOffTimeDiv = document.querySelectorAll(
            `#matches tr[data-id="${match.id}"] .kickoff-time`
          )[0];
          let loadingDiv = document.querySelectorAll(
            `#matches tr[data-id="${match.id}"] .loading`
          )[0];
            ...
            (result) => {
              ...
              if (streams.length > 0) {
                ...
                DomHelper.showInfoMessage(
                  'There are currently no available streams for this match'
                );
              }
              ...
              DomHelper.hideElement(DomHelper.soccerStreamsNote);
  ...
  populateStreamsTable(streams: Stream[]): void {
    for (let stream of streams) {
      ...
      for (let i = 0; i < stream.aceStreams.length; i++) {
        ...
        aceStreamButton.addEventListener(
          'click',
          () => {
      ...
      for (let i = 0; i < stream.sopCastStreams.length; i++) {
        ...
        sopCastStreamButton.addEventListener(
          'click',
          () => {
  ...
  filterMatches(searchText: string): void {
    ...
    for (let i = 0; i < matchesTr.length; i++) {
      let matchTitle: string = (<HTMLTableCellElement>matchesTr[i].children[1])
        .innerText;
